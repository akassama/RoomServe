...
class Personal_options_model extends Admin_Model
{
    public function __construct()
    {
        parent::__construct();
        $this->table = 'pls_personal_options';
    }
    ...
    */
    ...
    {
        $options['where']['pls_personal_options.is_deleted'] = 0;
        $options['where']['pls_personal_options.status !='] = STATUS_DRAFT;
        $options['group_by'] = ['pls_personal_options.personal_id'];
    ...
    function save($data)
    {
        if (isset($data['personal_id'])) {
            ...
            $result = $this->db->update($this->table, $data, 'personal_id = '.$data['personal_id']);
        }
        else {
    ...
    {
        $this->db->where([
            'pls_personal_options.personal_id' => $id
        ]);
     ...
     */
    public function get_all_personals($type = NULL){
        $options['as_array'] = TRUE;
        $options['fields'] = 'pls_personal_options.personal_id, name';
