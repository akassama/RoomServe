...
class Orders extends Admin_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->module_name = in_array($preset_name, [])?'orders_approval':'orders';
    ...
    */
    ...
    public function create() {
        if ($id = $this->orders_model->save(['created_by' => $this->user->user_id])){
    ...
    public function update($id = NULL, $lang = NULL) {
        ...
        if ($id && $data['data'] = $this->orders_model->load($id, $data['lang'])) {
            ...
            if ($post = $this->input->post('form')) {
                ...
                $fields = ['name', 'description', 'payment_type', 'order_date', 'student_id'];
                if ($order_data = $this->pls_validation_lib->validate('orders', $post, $fields)) {
                    ...
                    if($data['data']->student_id && ($data['data']->student_id == $post['student_id'])){
                        $order_data['student_id'] = $data['data']->student_id;
                    }
                    else{
                        $order_data['student_id'] = $post['student_id'];
                    }
                    ...
                    if ($this->orders_model->save($order_data) && $this->orders_model->save_translation($order_trans_data)) {
                        ...
                        $this->send_email($order_data, lang('email_theme_update'), lang('email_message_update'));
                    }
                    else {
    ...
    */
    ...
    public function send_email($order, $theme, $message){
        $student_id = isset($order->student_id)?$order->student_id:$order['student_id'];
    ...
    */
    ...
    public function get_ajax_creators()
    {
     ...
     //-------------------------------------------------------------------------
    public function get_ajax_customers()
    {
        if ($this->input->is_ajax_request()) {
            $options['fields'] = 'pls_orders.student_id as key, CONCAT(pls_users.first_name," ", COALESCE(pls_users.last_name, "")) as val';
            $options['group_by'] = 'pls_users.user_id';
            $options['order_by'] = ['key' => 'val', 'direction' => 'ASC'];
            echo json_encode($this->orders_model->get_grid_list($options));
        }
    ...
    private function prepare_order_data($id, $data, $for_approve = FALSE)
    {
        ...
        //common
        $data['all_customers'] = (array) $this->users_model->get_all_user_by_role(USER_ROLE_PARTNER_ADMINISTRATOR);
        ...
        $data['personnels'] = $this->personal_options_model->get_all_personals();
